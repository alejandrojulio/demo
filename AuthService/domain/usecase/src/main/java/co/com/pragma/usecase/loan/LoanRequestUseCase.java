...
import co.com.pragma.usecase.loan.LoanRequestUseCaseConstants;
...
public class LoanRequestUseCase {
    ...
    private Mono<LoanRequestDTO> validateClientExists(LoanRequestDTO dto) {
        logger.info(LoanRequestUseCaseConstants.LOG_VALIDATING_CLIENT_EXISTS, dto.getClientDocumentId());

        return userRepository.findByDocument(dto.getClientDocumentId().trim())
                .doOnSuccess(user -> {
                    if (user != null) {
                        logger.info(LoanRequestUseCaseConstants.LOG_CLIENT_VALIDATION_SUCCESS, dto.getClientDocumentId());
                    } else {
                        logger.info(LoanRequestUseCaseConstants.LOG_CLIENT_NOT_FOUND, dto.getClientDocumentId());
                    }
                })
                .flatMap(user -> {
                    if (user != null) {
                        ...
                        return Mono.error(new IllegalArgumentException(
                                String.format(LoanRequestUseCaseConstants.ERROR_CLIENT_NOT_FOUND, dto.getClientDocumentId())
                        ));
                    }
                })
